...
function doRectanglesOverlap(rect1, rect2) {
  return rect1.width - rect2.top >= 0 && rect1.height - rect2.left >= 0;
}
...
function findFirstSingleChar(str) {
  const charCount = Array.from(str).reduce((accumulator, char) => {
    accumulator[char] = (accumulator[char] || 0) + 1;
    return accumulator;
  }, {});

  return Array.from(str).find((char) => charCount[char] === 1) || null;
}
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < pathSegments.length; i += 1) {
    ...
    for (let j = 1; j < pathes.length; j += 1) {
      const currentPathSegments = pathes[j].split('/');

      if (currentPathSegments[i] !== segment) {
        return pathSegments.slice(0, i).join('/') + (i > 0 ? '/' : '');
      }
    }
  }

  return firstPath + (firstPath.endsWith('/') ? '' : '/');
}
...
function getMatrixProduct(m1, m2) {
  ...
  const product = Array.from({ length: rowsM1 }, () => Array(colsM2).fill(0));
...
function evaluateTicTacToePosition(position) {
  ...
  return winningCombinations.reduce((result, combination) => {
    if (result) return result;

    const [a, b, c] = combination;
    const [val1, val2, val3] = [position[a[0]][a[1]], position[b[0]][b[1]], position[c[0]][c[1]]];

    return val1 && val1 === val2 && val1 === val3 ? val1 : undefined;
  }, undefined);
}
